...
def float_prev(flt, popcount=1):
    if not flt in float_list:
        print("[DEBUG LOG] float_prev: flt not in float_list")
        return None
    i = float_list.index(flt)
    #print(f"[DEBUG LOG] {i} = float_list.index({flt})")
    if i < popcount:
        # Drop value (ex: FSTP ST(0))
        print("[DEBUG LOG] float_prev: i < popcount")
        return None
...
def fsubp(_, instr, dst, src=None):
    # fsub dst, src; pop dst
    dst, src = float_implicit_st0(dst, src)
    e = []
    src = mem2double(instr, src)
    print("[DEBUG LOG]", f"dst: {dst} | src: {src} | float_prev(dst): {float_prev(dst)}") # float_prev(dst) is None
    # temp modification
    if dst == float_st0:
        e.append(m2_expr.ExprAssign(dst, m2_expr.ExprOp('fsub', dst, src)))
    else:
