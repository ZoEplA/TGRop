...
def encode_ir_gadgets(iras: List[GadgetIRTy],
    ...
    # debug line: make iras unique by selecting iras[i][0] as a key
    unique_dict = {}
    for tup in iras:
        unique_dict[tup[0]] = tup[1]
    iras = [(k, v) for k, v in unique_dict.items()]
...
def run_config(synthesizer_config: SynthesizerConfig, target_config: TargetConfig,
    ...
    if solver_result == SMTResult.SAT:
        ...
        print(f"Time used until verification stage: {round(stats['time_solver'], 2)}s")
...
def prepare_caches(target: Path, workdir: Path, target_config: TargetConfig, disas_settings: DisasSettings) -> None:
        ...
        # 从这里计算gadget提取时间
        _gadget_extract_start = time.time()
        cache_gadgets(disas_settings)
        _gadget_extract_end = time.time()
        logger.debug(f"[expr3] Extract all gadgets from {target.name} in {round((_gadget_extract_start - _gadget_extract_end), 2)}s")
