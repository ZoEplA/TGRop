...
class ROPGadget:
    ...
    def run(self, timeout):
        ...
        pp = '/usr/bin/python2' # environ["PYTHONPATH"]
        ...
        with open(script_path, 'wb') as script:
            for gadget in ropchain_generator:
                script.write(b"\n" + gadget)
        ...
        with open(self.ropchain, "wb") as ropchain_output:
            ...
            script_p = Popen(script_cmd, stdout=PIPE, stderr=PIPE)
            try:
                # stderr = script_p.communicate(timeout=timeout)[1]
                stdout, stderr = script_p.communicate(timeout=timeout)
                ropchain_output.write(stdout.replace(b'/bin//sh', b'/bin//fh'))
