...
#        return "addr : {}\nwritten : {}\nread : {}\npopped : {}\ndepends : {}\ndiff_sp: {}\n".format(self.addr, self.written_regs, self.read_regs, self.popped_regs, self.depends_regs, self.diff_sp)
    ...
    def analyzeGadget(self, debug=False):
        ...
        while True:
            ...
            inst.setAddress(pc)
            if(ctx.processing(inst) == EXCEPTION.FAULT_UD):
                return False
            ...
            if inst.isControlFlow(): #  or 'ret' in inst.getDisassembly() or 'call' in inst.getDisassembly() or 'jmp' in inst.getDisassembly() : # check if end of gadget
                type_end = 0
